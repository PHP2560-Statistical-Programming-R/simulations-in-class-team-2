...
```{r}
busted.AR.hand = replicate(5000, GR.bust(1000, 100, 18/38, 5000))
bust.AR.a= length(which(busted.AR.hand < 100)) 
bust.total = 5000

prob.AR.a = bust.AR.a / bust.total
prob.AR.a
```
...
```{r}
bust.AR.b = length(which(busted.AR.hand < 500))

prob.AR.b = bust.AR.b / 5000
prob.AR.b
```
...
```{r}
busted.AR.time = busted.AR.hand[which(busted.AR.hand < 5000)]
mean(busted.AR.time)
```
...
```{r}
bankroll.AR.100 = replicate(5000, GR.bankroll(1000, 100, 18/38, 100))

mean(bankroll.AR.100)
var(bankroll.AR.100)
```
...
```{r}
bankroll.AR.500 = replicate(5000, GR.bankroll(1000, 100, 0.5, 500))

mean(bankroll.AR.500)
var(bankroll.AR.500)
```
...
```{r}
boot_ci = function(data, stat=mean, reps=1000, conf = c(.025, .975)){
  res = vector("numeric", length=reps)
  for(i in 1:reps){
  samp = sample(data, replace=T, size=length(data))
  res[i] = stat(samp)
  }
  return(quantile(res, conf))
}
```
...
```
=======
>>>>>>> 68e2e54c85a1ddf7f82b0bd5931c9387e6138c76
```{r}
#Our fair prob for 3b was .491
boot_ci(data=replicate(100, MC.bankroll(100, 0.491, 0.01, 100000)),stat=var)

#Our fair increment for 3c was .015
boot_ci(data=replicate(100, MC.bankroll(100, 0.48, 0.015, 100000)),stat=var)
```
Based on these results, the game from 3b (which uses .491 as a starting probability and a 1% increment) has the smaller variance in returns.

>>>>>>> e96c8762bedd090527dfc783e7e9ad9acd223b02
